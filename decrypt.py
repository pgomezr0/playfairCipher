...
@author: pgomezr0
'''
# COMPUTER SECURITY ASSIGNMENT 1 EXERCISE 2 - DECRYPT EXTENDED
...
def decrypt(text, key):
    ...
    while i < len(diagraphs):
        ...
        if n1[1] == n2[1]:
            ...
            i2 = (n2[0] - 1) % 5
            j2 = n2[1]
            decrypted_text.append(key_grid[i1][j1])
            decrypted_text.append(key_grid[i2][j2])
            decrypted_text.append(' ')

        # Rule 3: If pair of letters (diagraph) appears on the same row of key grid take the letter to the
        ...
        # exchange columns of both values
            ...
            decrypted_text.append(key_grid[i1][j2])
            decrypted_text.append(key_grid[i2][j1])
            decrypted_text.append(' ')
...
def main():
    ...
    print('\n********* PLAYFAIR CIPHER DECRYPTION EXTENDED************\n')

    user_text = input("Input the CIPHERTEXT you wish to decrypt: ")
    user_key = input("Input the KEY and press ENTER once you're done: ")

    user_text = user_text.replace(' ', '').upper()
    user_key = user_key.replace(' ', '').upper()

    decrypted_message = decrypt(user_text, user_key).replace(' ', '')

    file_message = open('plaintext_playfair.txt', 'w')
    file_message.write(decrypted_message)
    file_message.close()

    print('\nThe message has been decrypted.')
    print('The plaintext has been stored in a .txt file in the current folder.\n')
